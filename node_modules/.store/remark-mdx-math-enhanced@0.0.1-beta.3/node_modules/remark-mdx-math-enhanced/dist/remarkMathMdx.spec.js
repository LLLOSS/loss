...
describe('remarkMdxMathEnhancedPlugin', () => {
    it('should compile inline katex to HTML', () => {
        expect(unified()
            ...
            .toString()).toEqual(String.raw `Hey this is math <Math>{\frac{a}{b}}</Math>
`);
    });
    it('should compile display katex to HTML', () => {
        expect(unified()
            ...
            .use(remarkMdxMathEnhancedPlugin)
            .use(remarkStringify)
            .processSync(String.raw `
Hey this is math

$$
\frac{a}{b}
$$`)
            .toString()).toEqual(String.raw `Hey this is math
...
`);
    ...
    it('should compile inline katex with JS expressions to HTML', () => {
        expect(unified()
            ...
            .use(remarkMdxMathEnhancedPlugin)
            .use(remarkStringify)
            .processSync(String.raw `Hey this is math with JS $\pi = \js{Math.PI}$`)
            .toString()).toEqual(`Hey this is math with JS <Math>{\\pi = $\{Math.PI\}}</Math>
`);
    });
    it('should compile display katex with JS expressions', () => {
        expect(unified()
            ...
            .use(remarkMdxMathEnhancedPlugin)
            .use(remarkStringify)
            .processSync(String.raw `Hey this is math with JS

$$
\pi = \js{Math.PI}
$$
`)
            .toString()).toEqual(`Hey this is math with JS
...
`);
    ...
    it('should parse simple JS expressions', () => {
        expect(unified()
            .use(remarkParse)
            .use(remarkMdxMathEnhancedPlugin)
            .runSync(removePosition(unified()
            .use(remarkParse)
            .use(remarkMath)
            .parse(String.raw `
$\pi = \js{Math.PI}$
...
`)
    ...
    it('should allow custom component name', () => {
        expect(unified()
            ...
            .use(remarkStringify)
            .processSync(String.raw `Hey this is math with JS $\pi = \js{Math.PI}$`)
            .toString()).toEqual(`Hey this is math with JS <CustomMath>{\\pi = $\{Math.PI\}}</CustomMath>
`);
    });
    it('should allow custom expressionMarker', () => {
        expect(unified()
            ...
            .use(remarkStringify)
            .processSync(String.raw `Hey this is math with JS $\pi = [[Math.PI]]$`)
            .toString()).toEqual(`Hey this is math with JS <Math>{\\pi = $\{Math.PI\}}</Math>
`);
    });
});
